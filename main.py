...
def save_pass():
    ...
    if exists1:
        ...
        if new_pas == None:
            ...
            tf = open(os.path.join('TrainingImageLabel','psd.txt'), "w")
            tf.write(new_pas)
    ...
    if (op == key):
        if(newp == nnewp):
            txf = open(os.path.join('TrainingImageLabel','psd.txt'), "w")
            txf.write(newp)
        else:
...
def TakeImages():
    ...
    if ((name.isalpha()) or (' ' in name)):
        ...
        harcascadePath = "Face Recognition Based Attendance Monitoring System\haarcascade_frontalface_default.xml"
...
def TrackImages():
    ...
    exists1 = os.path.isfile(os.path.join('StudentDetails','StudentDetails.csv'))
    if exists1:
